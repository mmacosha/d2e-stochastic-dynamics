...
def compute_fwd_tb_log_difference(fwd_model, bwd_model, log_p1, x, 
    ...
    x1 = xt_m_dt
    log_diff = log_p_bwd + log_p1(x1) - log_p_fwd
...
def compute_bwd_tb_log_difference(fwd_model, bwd_model, log_p, x, dt, t_max, 
    ...
    for t_step in torch.linspace(dt, t_max, num_t_steps).flip(-1):
        ...
        with torch.set_grad_enabled(learn_bwd):
            ...
            with torch.no_grad():
                xt_m_dt = bwd_mean + \
                          bwd_log_var.exp().sqrt() * torch.randn_like(bwd_mean)
        ...
        with torch.set_grad_enabled(not learn_bwd):
            fwd_mean, fwd_log_var = \
                utils.get_mean_log_var(fwd_model, xt_m_dt, t - dt, dt)
            log_p_fwd = log_p_fwd + \
    ...
    if learn_bwd:
        log_p_fwd = log_p_fwd + log_p(xt)

    if p0_buffer is not None:
        p0_buffer.update(xt)
    
    if return_x:
        return log_p_fwd - log_p_bwd, xt
    
    return  log_p_fwd - log_p_bwd


def compute_fwd_tb_loss(fwd_model, bwd_model, log_p1, log_p0, x, dt, t_max, 
                        num_t_steps, p1_buffer = None):
    log = compute_fwd_tb_log_difference(
        fwd_model, bwd_model, 
        log_p1, x, dt, t_max, num_t_steps, 
        p1_buffer=p1_buffer
    )
    log = log - log_p0(x)
...
def compute_bwd_vargrad_loss(fwd_model, bwd_model, log_p0, x, dt, t_max,
    ...
    log = compute_bwd_tb_log_difference(
        fwd_model, bwd_model, 
        log_p0, x, dt, t_max, num_t_steps, 
        p0_buffer=p0_buffer
     )
    log = log.reshape(n_trajectories, -1)
    return (log  - log.mean(0, keepdim=True).detach()).pow(2).mean()
...
def compute_fwd_tb_log_difference_reuse_traj(
    ...
    xt = x
    log_p_fwd, log_p_bwd = 0, log_p1(xt)
